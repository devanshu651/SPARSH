...
export const demoApi = {
  ...
  children: {
    ...
    create: async (data) => { const child={...data,id:`demo-child-${Date.now()}`,age_months:Math.max(0, Math.floor((Date.now()-new Date(data.date_of_birth))/2629800000)),created_at:new Date().toISOString(),created_by:'demo-worker',latest_risk:null}; const items=[...read('children', []),child];write('children',items);return child },
    healthData: async (id,data) => { const all=read('health',{});const entry={...data,id:`demo-health-${Date.now()}`,child_id:id,created_at:new Date().toISOString()};write('health',{...all,[id]:[...(all[id]||[]),entry]});return entry },
    healthHistory: async id => read('health',{})[id] || [],
    history: async id => ({child_id:id,screenings:read('screenings',{} )[id] || []}),
  },
  screenings: {
    milestones: async childId => { const child=read('children', []).find(x=>x.id===childId);const age=Math.max(0,Math.floor((Date.now()-new Date(child?.date_of_birth||Date.now()))/2629800000));return {dataset_version:'demo',current_age_months:age,checkpoint_age_months:age,milestones} },
