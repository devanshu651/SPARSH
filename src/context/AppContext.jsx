...
export function AppProvider({ children }) {
  ...
  useEffect(() => authService.observeAuthState((user) => {
    setAuthReady(true)
    if (!user && !isDemoMode()) setCurrentWorker(null)
  }), [])
    ...
    const sync = async () => {
      ...
      try {
        const safeSubmit = async (payload) => {
          const call = screeningsApi.submit(payload)
          return typeof call === 'function' ? await call(payload) : await call
        }
        await syncQueuedScreenings(safeSubmit)
      } catch {}
    }
