...
export function AppProvider({children}){const [currentWorker,setCurrentWorker]=useState(()=>isDemoMode()?demoWorker:null);const [currentChild,setCurrentChild]=useState(null);const [screeningResult,setScreeningResult]=useState(null);const [authReady,setAuthReady]=useState(false);useEffect(()=>authService.observeAuthState(user=>{setAuthReady(true);if(!user&&!isDemoMode())setCurrentWorker(null)}),[]);useEffect(()=>{const sync=async()=>{if(!navigator.onLine||isDemoMode())return;const remaining=[];for(const item of queuedScreenings()){try{await screeningsApi.submit(item)}catch{remaining.push(item)}}setQueue(remaining)};window.addEventListener('online',sync);sync();return()=>window.removeEventListener('online',sync)},[]);return <AppContext.Provider value={{currentWorker,setCurrentWorker,currentChild,setCurrentChild,screeningResult,setScreeningResult,authReady,demoWorker}}>{children}</AppContext.Provider>}
