...
def age_months(dob: date) -> int:
    today = date.today(); return max(0, (today.year-dob.year)*12 + today.month-dob.month - (today.day < dob.day))
def child_from_snapshot(snapshot) -> dict:
    data = snapshot.to_dict()
    if not data: raise HTTPException(status_code=404, detail="Child not found")
    data["id"] = snapshot.id; return data
...
def register_child(payload: ChildCreate, user: CurrentUser = Depends(require_roles(Role.WORKER, Role.ADMIN))):
    ...
    now = datetime.now(timezone.utc); data = payload.model_dump() | {"age_months": age_months(payload.date_of_birth), "created_at": now, "created_by": user.uid}
    ref = get_firestore_client().collection("children").document(); ref.set(data)
    return ChildResponse(id=ref.id, **data)
...
def child_milestones(child_id: str, user: CurrentUser = Depends(get_current_user)):
    """Return the server-calculated age and checkpoint for one child."""
...
def get_child(child_id: str, user: CurrentUser = Depends(get_current_user)):
    child = child_from_snapshot(get_firestore_client().collection("children").document(child_id).get()); ensure_centre_access(user, child["centre_id"]); return ChildResponse(**child)
...
def record_health_data(child_id: str, payload: HealthDataCreate, user: CurrentUser = Depends(require_roles(Role.WORKER, Role.ADMIN))):
    child = child_from_snapshot(get_firestore_client().collection("children").document(child_id).get()); ensure_centre_access(user, child["centre_id"])
    data = payload.model_dump() | {"child_id": child_id, "recorded_by": user.uid, "created_at": datetime.now(timezone.utc)}
    ref = get_firestore_client().collection("children").document(child_id).collection("health_data").document(); ref.set(data)
